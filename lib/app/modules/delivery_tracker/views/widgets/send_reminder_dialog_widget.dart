...
class SendReminderDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      ...
      final totalPending = controller.pendingAgents.length;
      final agentCount = selectedCount > 0 ? selectedCount : totalPending;
      final message = selectedCount > 0
          ? 'Send reminder to $agentCount selected agent${agentCount == 1 ? '' : 's'}?'
          : 'Send reminder to all $agentCount pending agent${agentCount == 1 ? '' : 's'}?';
